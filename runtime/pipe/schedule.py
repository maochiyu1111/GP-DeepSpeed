...
class TrainSchedule(PipeSchedule):
    ...
    """
    #wangjin
    def __init__(self, micro_batches, stages, stage_id, global_steps = 0):
    ...
    def steps(self):
        ...
        for step_id in range(total_steps):
            ...
            cmds = []
            #wangjin 直接加一个encoder
            if step_id == 0:
                if self.global_steps == 0:  # 只在第一个step 加
                    cmds.append(CalculateEncoder(self.stage_id))
                cmds.append(SendEncoderRes(self.stage_id))
            ...
            #Wangjin 看起来这里是想要在最后一个stage load lables, 先跳过
            # First/last stage loads
            if self.stage_id == 0 and step_id == 0: # or self.stage_id == self.stages - 1:
                if is_forward and self._valid_micro_batch(micro_batch_id):
                    #wangjin 
                    # cmds.append(LoadMicroBatch(curr_buffer))
                    cmds.append(RecvEncoderRes(curr_buffer))
                    #end
            # Computation
            if self._valid_micro_batch(micro_batch_id):
                if is_forward:
            ...
            if step_id == total_steps - 1:
                ...
                encoder_count = 0
                # for i in range(self.getEncoderSteps(self.stage_id)):
                #     cmds.append(CalculateEncoder(encoder_count))
                #     encoder_count += 1
                cmds.append(FinishEncoder())
                cmds.append(OptimizerStep())

            # Prepare state for next time
            prev_micro_batch_id = micro_batch_id
            # if step_id == 1 and self.stage_id == 0:
            #     cmds = [RecvEncoderRes(1)]
            # if step_id == 2 and self.stage_id == 0:
            #     cmds = [SendActivation(0),ForwardPass(curr_buffer)]
            yield cmds

    def getEncoderSteps(self, stage_id):
        if stage_id == self.num_stages -1:
            return self.num_stages - 2
        if stage_id == self.num_stages - 2:
            return 2
        return 0
...
#Wangjin
class CalculateEncoder(BufferOpInstruction):
    pass
class SendEncoderRes(BufferOpInstruction):
    pass

class RecvEncoderRes(BufferOpInstruction):
    pass

class FinishEncoder(PipeInstruction):
    pass
#end
