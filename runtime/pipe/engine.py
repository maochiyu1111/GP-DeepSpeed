...
# 类装饰器：为类中的所有方法加上print_function_name装饰器
def decorate_all_methods(decorator):
    def class_decorator(cls):
        for attr_name, attr_value in cls.__dict__.items():
            if callable(attr_value):  # 判断是否是可调用对象（即函数或方法）
                setattr(cls, attr_name, decorator(attr_value))  # 给方法加装饰器
        return cls
...
# @decorate_all_methods(print_function_name)
class PipelineEngine(DeepSpeedEngine):
    ...
    def _reserve_pipe_buffers(self, num_buffers):
        ...
        #wangjin
        if len(self.pipe_buffers["encoder_res"]) < self.num_stages:
            self.pipe_buffers["encoder_res"].extend([None] * self.num_stages)
    ...
    def train_batch(self, data_iter=None):
        ...
        self._exec_schedule(self.sched)
    ...
    def _exec_forward_pass(self, buffer_id):
        print("_exec_forward_pass")
        self.tput_timer.start()
        self.mem_status('BEFORE FWD', reset_max=True)

        if isinstance(self.pipe_buffers['inputs'][buffer_id], tuple):
            inputs = tuple(t.clone() if t is not None else torch.tensor(0).to(self.device) for t in self.pipe_buffers['inputs'][buffer_id])
        else:
    ...
    def _send_tensor_meta(self, buffer, recv_stage, lockEncoder = False):
        ...
        if isinstance(buffer, torch.Tensor):
            ...
            if lockEncoder:
                ...
                work = p2p.send_wangjinAsync(type_tensor, recv_stage)
                while not work.is_completed():
                    res = self._try_calculate_encoder()
                    if res:
                        break
                work.wait()
            #wangjin
            ...
            #wangjin
            if lockEncoder:
                type_tensor = torch.LongTensor(data=[2]).to(self.device, non_blocking= False)
                p2p.send(type_tensor, recv_stage)
            else:
                type_tensor = torch.LongTensor(data=[2]).to(self.device, non_blocking= True)
                eventwj = torch.cuda.Event()
                eventwj.record()
                compute_stream = torch.cuda.Stream()

                while not eventwj.query():
    ...
    def _recv_tensor_meta(self, send_stage, lockEncoder = False):
        ...
        if lockEncoder:
            ...
            work =  p2p.recv_wangjinAsync(type_tensor, send_stage)
            while not work.is_completed():
                res = self._try_calculate_encoder()
                if res:
                    break
...
#Wangjin
    def _exec_cal_encoder(self, buffer_id):
        #get batch first
        if self.wall_clock_breakdown():
            self.timers('_exec_cal_encoder').start()

        # buffer_id = self.grid.get_global_rank()
        ...
        loaded = self.module.encoder(loaded)

        loaded = tuple([t if t is not None else torch.tensor(0).to(self.device) for t in loaded])

        if self.is_first_stage():
            self.pipe_buffers['inputs'][buffer_id] = loaded
        else:
            ##save
            self.pipe_buffers['encoder_res'][buffer_id] = loaded
        
        if self.wall_clock_breakdown():
            self.timers('_exec_cal_encoder').stop()
           
    def _exec_send_encoder(self, buffer_id):
        if self.stage_id == 0:
            return
        if self.wall_clock_breakdown():
            self.timers('_exec_send_encoder').start()
    #Wangjin 
            # if self.first_output_send:
            #     self.first_output_send = False
            #     self._send_tensor_meta(outputs, self.next_stage)
    ###################################################################
        for i in range(self.num_stages):
            if self.pipe_buffers['encoder_res'][i] is not None:
                outputs = self.pipe_buffers['encoder_res'][i]
                self._send_tensor_meta(outputs, 0, True)
                if isinstance(outputs, torch.Tensor):
                    p2p.send(outputs, 0)
                elif isinstance(outputs, tuple):
                    for idx, buffer in enumerate(outputs):
                    ...
                    raise NotImplementedError('Could not send output of type '
                                            f'{type(outputs)}')
                self.pipe_buffers['encoder_res'][i] = None


        if self.wall_clock_breakdown():
            self.timers('_exec_send_encoder').stop()
        ## send

    def _exec_recv_encoder(self, buffer_id):
        
        # if self.global_steps == 0:
        for i in range(self.num_stages):
            self._exec_recv_encoder_internal(i,i)
        # else:
        #     buffer_id = 0
        #     for i in range(self.num_stages):
        #         for j in range(self.sched.getEncoderSteps(i)):
        #             self._exec_recv_encoder_internal(i,buffer_id)
        #             buffer_id += 1
        # self._exec_recv_encoder_internal(1,0) 
        # self._exec_recv_encoder_internal(2,1)
        # self._exec_recv_encoder_internal(3,2)
        # self._exec_recv_encoder_internal(3,3)

    def _exec_recv_encoder_internal(self, stage_id, buffer_id):
        
        if stage_id == 0:
            return
        if self.wall_clock_breakdown():
            self.timers('_exec_recv_encoder').start()
        recvd = None
        #Wangjin modified Begin
        # Allocate the buffer if necessary
        # if self.pipe_recv_buf is None:
        #     self.pipe_recv_buf = self._recv_tensor_meta(self.prev_stage)
##################################################################################
        self.pipe_recv_buf = self._recv_tensor_meta(stage_id, True)
        #End

        if isinstance(self.pipe_recv_buf, torch.Tensor):
            p2p.recv(self.pipe_recv_buf, stage_id)
            recvd = self.pipe_recv_buf.clone().detach()
            recvd.requires_grad = recvd.is_floating_point()
        else:
            assert isinstance(self.pipe_recv_buf, tuple)
        ...
        if self.wall_clock_breakdown():
            self.timers('_exec_recv_encoder').stop()
#end

    def _try_calculate_encoder(self, bFinish = False):
        if self.module.encoder.is_first_stage():
            batch = self._next_batch()
            loaded = None
            if torch.is_tensor(batch[0]):
                loaded = batch[0].clone().to(self.device).detach()
                ...
                loaded = tuple(loaded)
            if bFinish:
                return self.module.encoder.finish(loaded)
            else:
                return self.module.encoder.next(loaded)
        else:
            if bFinish:
                return self.module.encoder.finish()
            else:
                return self.module.encoder.next()
    
    def _try_finish_encoder(self):
        self._try_calculate_encoder(True)

        loaded = self.module.encoder.getRes()
        self.module.encoder.reset()

        loaded = tuple([t if t is not None else torch.tensor(0).to(self.device) for t in loaded])

        if self.is_first_stage():
            self.pipe_buffers['inputs'][0] = loaded
        else:
            ##save
            self.pipe_buffers['encoder_res'][0] = loaded
    ...
    _INSTRUCTION_MAP = {
        ...
        schedule.CalculateEncoder:_exec_cal_encoder,
        schedule.SendEncoderRes: _exec_send_encoder,
        schedule.RecvEncoderRes: _exec_recv_encoder,
        schedule.FinishEncoder: _try_finish_encoder,
    }
