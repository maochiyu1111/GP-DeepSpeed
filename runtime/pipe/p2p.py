...
def _is_valid_send_recv(src_stage, dest_stage):
    ...
    assert abs(src_stage-dest_stage) == 1 or \
        ...
        (src_stage == last_stage and dest_stage == first_stage) or (dest_stage == first_stage), \
...
def send_wangjinAsync(tensor, dest_stage):
    global _groups
    src_stage = _grid.get_stage_id()
    _is_valid_send_recv(src_stage, dest_stage)
    ...
    dest_rank = _grid.stage_to_global(stage_id=dest_stage)
    if can_send_recv():
        return torch.distributed.isend(tensor=tensor, dst=dest_rank)
    else:
        assert False, "send_wangjinAsync"

def recv_wangjinAsync(tensor, src_stage):
    global _groups
    ...
    if can_send_recv():
        return torch.distributed.irecv(tensor=tensor, src=src_rank)
    else:
        assert False, "recv_wangjinAsync"
